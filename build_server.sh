...
mkdir -p "$SIF_DIR" 2>/dev/null || {
    ...
    print_error "You may need to run: sudo mkdir -p $SIF_DIR && sudo chown $USER:$USER $SIF_DIR"
    exit 1
}

mkdir -p "$TMP_DIR" 2>/dev/null || {
    print_error "Cannot create temp directory: $TMP_DIR"
    print_error "You may need to run: sudo mkdir -p $TMP_DIR && sudo chown $USER:$USER $TMP_DIR"
    exit 1
}
...
if [ ! -w "$SIF_DIR" ]; then
    ...
    print_error "Run: sudo chown $USER:$USER $SIF_DIR"
    exit 1
fi

if [ ! -w "$TMP_DIR" ]; then
    print_error "No write permission for temp directory: $TMP_DIR"
    print_error "Run: sudo chown $USER:$USER $TMP_DIR"
    exit 1
fi
