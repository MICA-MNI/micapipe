...
for (annot in c('aparc', 'aparc.a2009s')) {
  nom <- paste0(BIDSid,"_space-fsnative_",annot,"_desc-qc.png"); nom.png <- paste0(dir_QC_png,"/",nom)
  if (!file.exists(nom.png)) {
    ...
    img <- vis.export.from.coloredmeshes(cm, output_img = nom.png, img_only = TRUE, grid_like = FALSE, view_angles = view_angles);
...
####  fs - Sulcal depth ####
nom <-  paste0(BIDSid,"_space-fsnative_desc-qc_sulc.png"); nom.png <- paste0(dir_QC_png,"/",nom)
if (!file.exists(nom.png)) {
  print(paste0("INFO.... Creating PNG of the SULCAL DEPTH on native surface"))
  rgla <- list('trans_fun'=limit_fun(-5, 5), 'no_vis'=T);
  cm <- vis.subject.morph.native(dir_fs, BIDSid, 'sulc', cortex_only = FALSE,
                                 draw_colorbar = TRUE, surface="white", rglactions = rgla, makecmap_options = list('colFn'=cividis))
  img <- vis.export.from.coloredmeshes(cm, output_img = nom.png, colorbar_legend = 'Sulcal depth [mm]', grid_like = FALSE, view_angles = view_angles);
  while (rgl.cur() > 0) { rgl.close() }
} else { print(paste0("INFO.... File exists: ", nom)) }


####  fs - Cortical thickness ####
nom <-  paste0(BIDSid,"_space-fsnative_desc-qc_thickness.png"); nom.png <- paste0(dir_QC_png,"/",nom)
if (!file.exists(nom.png)) {
  print(paste0("INFO.... Creating PNG of the THICKNESS on native surface"))
  rgla <- list('trans_fun'=limit_fun(1.5, 4), 'no_vis'=T);
  cm <- vis.subject.morph.native(dir_fs, BIDSid, 'thickness', cortex_only = FALSE,
...
nom <-  paste0(BIDSid,"_space-fsnative_desc-qc_curv.png"); nom.png <- paste0(dir_QC_png,"/",nom)
if (!file.exists(nom.png)) {
  print(paste0("INFO.... Creating PNG of the CURVATURE on native surface"))
  rgla <- list('trans_fun'=limit_fun(-0.2, 0.2), 'no_vis'=T);
  cm <- vis.subject.morph.native(dir_fs, BIDSid, 'curv', cortex_only = FALSE,
