...
def get_feature_array(surf, Save=True):
    ...
    if Save==True:
        fileName="{output}{bids_id}_surf-{surf}_desc-intensity_profiles_swm.shape.gii".format(output=OPATH, bids_id=bids_id, surf=surf)
...
fileName="{output}{bids_id}_surf-fsLR-5k_desc-MPC_swm.shape.gii".format(output=OPATH, bids_id=bids_id)
