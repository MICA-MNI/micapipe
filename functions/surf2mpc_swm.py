...
if os.path.exists(OPATH):
    try:
        ...
        if np.isnan(np.sum(MPC)):
            ...
            save_gii(MPC, "{output}/{bids_id}_atlas-{parc_str}_desc-MPC_swm.shape.gii".format(output=OPATH, bids_id=bids_id, parc_str=parc_str))
            save_gii(I, "{output}/{bids_id}_atlas-{parc_str}_desc-intensity_profiles_swm.shape.gii".format(output=OPATH, bids_id=bids_id, parc_str=parc_str))
            print("")
            print("-------------------------------------")
